...
class TournamentView(ttk.Frame):
    def __init__(self, parent, start_cb, stop_cb):
        ...
        self.table_frames = []
        self.player_labels = {}  # player_id -> (id_label, stack_label)
    ...
    def build_tables(self, num_tables, players_per_table):
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                ...
                # placeholders (id only; stacks will show once updates arrive)
                for p in range(players_per_table):
                    player_id = idx * players_per_table + p + 1
                    cell = ttk.Frame(tf)
                    cell.grid(row=p // 5, column=p % 5, padx=2, pady=2, sticky="n")
                    id_lbl = ttk.Label(cell, text=str(player_id), relief=tk.GROOVE, width=4)
                    id_lbl.pack()
                    stack_lbl = ttk.Label(cell, text="", width=6)
                    stack_lbl.pack()
                    self.player_labels[player_id] = (id_lbl, stack_lbl)
                idx += 1

    def update_tables(self, tables):
        # tables: list of lists of player dicts: [{"id":..., "stack":...}, ...]
        ...
        for ti, tf in enumerate(self.table_frames):
            ...
            for idx, pinfo in enumerate(players):
                pid = pinfo["id"] if isinstance(pinfo, dict) else pinfo
                stack = pinfo.get("stack", None) if isinstance(pinfo, dict) else None
                cell = ttk.Frame(tf)
                cell.grid(row=idx // 5, column=idx % 5, padx=2, pady=2, sticky="n")
                id_lbl = ttk.Label(cell, text=str(pid), relief=tk.GROOVE, width=4)
                id_lbl.pack()
                stack_lbl = ttk.Label(cell, text=f"${int(stack)}" if stack is not None else "", width=6)
                stack_lbl.pack()
                self.player_labels[pid] = (id_lbl, stack_lbl)

    def eliminate_player(self, player_id):
        lbls = self.player_labels.get(player_id)
        if lbls:
            id_lbl, stack_lbl = lbls
            id_lbl.config(text="", relief=tk.FLAT)
            stack_lbl.config(text="")
...
class ResultsView(ttk.Frame):
    ...
    def display_ranges(self, ranges):
        ...
        for pos in ["early", "middle", "late"]:
            if pos in post:
                self.text.insert(tk.END, fmt_matrix(f"Postflop ({pos})", post[pos]))inter as tk
from tkinter import ttk
    ...
    def build_tables(self, num_tables, players_per_table):
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                ...
                for p in range(players_per_table):
                    player_id = idx * players_per_table + p + 1
                    # Use multi-line label to show ID and stack placeholder
                    lbl = ttk.Label(tf, text=f"{player_id}\n$1000", relief=tk.GROOVE, width=8, anchor="center", justify="center")
                    lbl.grid(row=(p // 5)*2, column=p % 5, padx=2, pady=1)
                    self.player_labels[player_id] = lbl
                idx += 1

    def update_tables(self, tables):
        # tables: list of lists of player dicts or ids
        # Clear current and rebuild labels per table
        for tf in self.table_frames:
            for child in tf.winfo_children():
                child.destroy()
        ...
        for ti, tf in enumerate(self.table_frames):
            ...
            for idx, entry in enumerate(players):
                ...
                lbl.grid(row=(idx // 5)*2, column=idx % 5, padx=2, pady=1)
